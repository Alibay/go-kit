...
var (
	ErrCfgRead = func(cause error, path string) error {
		return kit.NewAppErrBuilder(ErrCodeCfgRead, "read config error: %s", path).Wrap(cause).Err()
	}
	ErrCfgMerge = func(cause error, path string) error {
		return kit.NewAppErrBuilder(ErrCodeCfgMerge, "merge config error: %s", path).Wrap(cause).Err()
	}
	ErrCfgUnmarshal = func(cause error) error {
		return kit.NewAppErrBuilder(ErrCodeCfgUnmarshal, "merge config error").Wrap(cause).Err()
	}
	ErrNoPathSpecified = func() error {
		return kit.NewAppErrBuilder(ErrCodeNoPathSpecified, "neither env nor explicit path is specified").Business().Err()
	}
	ErrConfigNotLoaded = func() error {
		return kit.NewAppErrBuilder(ErrCodeConfigNotLoaded, "config not loaded").Business().Err()
	}
)
