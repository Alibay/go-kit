...
func NewMigration(db *sql.DB, source string, log logger.CLoggerFunc, dialect string) Migration {
	return &migImpl{
		db:      db,
		source:  source,
		log:     log,
...
func (m *migImpl) l() logger.CLogger {
	return m.log().Cmp("db-mig")
}
...
func (m *migImpl) exec(fn func() error, l logger.CLogger) error {
	...
	if m.dialect == DialectPostgres {
		...
		defer func() {
			if _, err := m.db.Exec(fmt.Sprintf("select pg_advisory_unlock(%d)", pgMigrationAdvisoryLockId)); err != nil {
				m.log().E(ErrGooseMigrationUnLock(err)).Err()
			}
		}()
	}
