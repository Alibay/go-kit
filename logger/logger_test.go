...
func Test_Clogger_WithCtx(t *testing.T) {
	...
	ctx := kit.NewRequestCtx().WithNewRequestId().WithUser("1", "john").ToContext(context.Background())
	l := L(logger).C(ctx)
	l.Inf("I'm logger")
}
...
func Test_Clogger_All(t *testing.T) {
	logger := InitLogger(&LogConfig{Level: TraceLevel})
	ctx := kit.NewRequestCtx().WithNewRequestId().WithUser("1", "john").ToContext(context.Background())
	l := L(logger).Cmp("service").Mth("do").C(ctx).F(kit.KV{"field": "value"})
	l.Inf("I'm logger")
}

func Test_Clogger_WithFields(t *testing.T) {
	logger := InitLogger(&LogConfig{Level: TraceLevel})
	l := L(logger).F(kit.KV{"field": "value"})
	l.Inf("I'm logger")
}
...
func Test_Clogger_WithAppErr(t *testing.T) {
	logger := InitLogger(&LogConfig{Level: TraceLevel})
	l := L(logger).E(kit.NewAppError("ERR-123", "%s happened", "shit"))
	l.Err("my bad")
}

func Test_Clogger_WithAppErrAndStack(t *testing.T) {
	logger := InitLogger(&LogConfig{Level: TraceLevel})
	l := L(logger).E(kit.NewAppError("ERR-123", "%s happened", "shit")).St()
	l.Err("my bad")
}
...
func Test_Clogger_WithAppErrAndFields(t *testing.T) {
	logger := InitLogger(&LogConfig{Level: TraceLevel})
	e := kit.NewAppErrBuilder("ERR-123", "%s happened", "shit").F(kit.KV{"f": "v"}).Err()
	l := L(logger).E(e)
	l.Err("my bad")
}
...
func Test_Clogger_WithAppErrAndAppContext(t *testing.T) {
	logger := InitLogger(&LogConfig{Level: TraceLevel})
	ctx := kit.NewRequestCtx().WithRequestId("123").ToContext(context.Background())
	e := kit.NewAppErrBuilder("ERR-123", "%s happened", "shit").C(ctx).Err()
	l := L(logger).E(e)
	l.Err("my bad")
}
