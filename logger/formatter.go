...
type Formatter struct {
	// FixedFields - default: fields sorted alphabetically
	FixedFields []string
	...
	// HideKeysForFixedFields - shows fixed fields as [value]
	HideKeysForFixedFields bool
...
func (f *Formatter) writeOrderedFields(b *bytes.Buffer, entry *logrus.Entry) {
	...
	for _, field := range f.FixedFields {
...
func (f *Formatter) writeField(b *bytes.Buffer, entry *logrus.Entry, field string) {
	...
	if f.HideKeysForFixedFields {
		for _, f := range f.FixedFields {
			if field == f {
				hideKey = true
				break
			}
